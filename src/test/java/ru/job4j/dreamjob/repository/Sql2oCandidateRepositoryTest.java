...
class Sql2oCandidateRepositoryTest {
    ...
    public static void initRepositories() throws Exception {
        ...
        file = new File("test", UUID.randomUUID().toString());
    ...
    public void whenSaveSeveralThenGetAll() {
        var candidate1 = sql2oCandidateRepository.save(new Candidate(0, "name1", "description", 1, file.getId()));
        var candidate2 = sql2oCandidateRepository.save(new Candidate(0, "name2", "description", 1, file.getId()));
        var candidate3 = sql2oCandidateRepository.save(new Candidate(0, "name3", "description", 1, file.getId()));
        var result = sql2oCandidateRepository.findAll();
        assertThat(result).usingRecursiveComparison().isEqualTo(List.of(candidate1, candidate2, candidate3));
    }
    ...
    public void whenUpdateThenGetUpdated() {
        var candidate = sql2oCandidateRepository.save(new Candidate(0, "name", "description", 1, file.getId()));
        var updatedCandidate = new Candidate(candidate.getId(), "new name", "new description", 1, file.getId());
